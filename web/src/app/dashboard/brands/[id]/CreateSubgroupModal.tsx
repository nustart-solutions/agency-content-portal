...
export default function CreateSubgroupModal({ brandId, groupId, iconOnly = false }: { brandId: string, groupId: string, iconOnly?: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await createCampaignSubgroup(brandId, groupId, formData)
      if (result?.error) {
        ...
        <div className="modal-overlay">
          <div className="modal-content glass-panel" style={{ padding: '2rem' }}>
            ...
            <p style={{ color: 'var(--muted)', marginBottom: '1.5rem' }}>
              Subgroups organize specific verticals inside the Group (e.g., "Social Media", "Email Marketing").
            </p>

            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Subgroup Name</label>
                <input
                  type="text"
                  name="name"
                  placeholder="e.g. SEO Articles"
